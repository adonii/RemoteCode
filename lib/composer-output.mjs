...
import { bubbleToResponseMarkdown, todosToMarkdown } from './composer-response-markup.mjs';
 ...
 */
async function readComposerTodosAsync(composerId) {
  const key = escapeSqlString(`composerData:${composerId}`);
  const rows = await runSqliteJsonAsync(
    `SELECT json_extract(value,'$.todos') AS todos FROM cursorDiskKV WHERE key='${key}' LIMIT 1;`,
  );

  if (rows.length === 0 || rows[0].todos == null) {
    return null;
  }

  if (typeof rows[0].todos === 'string') {
    try {
      const parsed = JSON.parse(rows[0].todos);
  ...
  return Array.isArray(rows[0].todos) ? rows[0].todos : null;
}

/**
 * Extract the assistant's visible response for the latest turn (everything after
 * the last user message) as markdown, including prose, todo checkboxes, and
 * AskQuestion controls shown in the agent tab.
 *
 * @param {string} composerId
...
export async function readComposerResponseTextAsync(composerId) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        bubbleById.set(bubbleId, /** @type {Record<string, unknown>} */ (parsed));
      }
    } catch {
      // Skip malformed bubble blobs.
    }
  }
  ...
  for (const bubbleId of assistantBubbleIds) {
    ...
    if (
      toolFormerData &&
      typeof toolFormerData === 'object' &&
      !Array.isArray(toolFormerData) &&
      typeof /** @type {Record<string, unknown>} */ (toolFormerData).name === 'string' &&
      /** @type {Record<string, unknown>} */ (toolFormerData).name.toLowerCase() === 'todo_write'
    ) {
      sawTodoWrite = true;
    }

    const markdown = bubbleToResponseMarkdown(bubble);
    if (markdown) {
      parts.push(markdown);
    }
  }

  const composerTodos = await readComposerTodosAsync(composerId);
  if (composerTodos && composerTodos.length > 0) {
    const composerTodosMarkdown = todosToMarkdown(composerTodos, {
      heading: sawTodoWrite ? undefined : '### Tasks',
    });
    if (composerTodosMarkdown) {
      const lastPart = parts.at(-1) ?? '';
      if (lastPart.trim() !== composerTodosMarkdown.trim()) {
        parts.push(
          sawTodoWrite
            ? todosToMarkdown(composerTodos, { heading: '### Current tasks' })
            : composerTodosMarkdown,
        );
      }
    }
  }
