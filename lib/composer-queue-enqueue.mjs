...
const RESTORE_DELAYS_MS = [5_000, 12_000, 20_000];
const GENERATION_WAIT_MS = 15_000;
...
async function waitForComposerGeneration(composerId) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < GENERATION_WAIT_MS) {
    const signals = await readComposerSignalsAsync(composerId);
    if (signals.generatingCount > 0 || signals.headerCount > 0) {
      return { signals, waitedMs: Date.now() - startedAt };
    }
    await sleep(250);
  }
...
export async function enqueuePromptOnFocusedAgentTab(vscode, prompt) {
  ...
  const generation = await waitForComposerGeneration(composerId);

  log(
    `Post-create DB read for ${composerId}: status=${signals.status ?? 'MISSING'}, ` +
      `generatingCount=${generation.signals.generatingCount}, headers=${generation.signals.headerCount}, ` +
      `registered=${signals.present}, waitedMs=${waitedMs}, generationWaitMs=${generation.waitedMs}, ` +
