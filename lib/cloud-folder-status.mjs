...
import { APP_NAME, CLOUD_ROOT_FOLDER, ICLOUD_CONTAINER_ID } from './constants.mjs';
...
const FOLDER_STATUS_ICLOUD_CACHE_MS = 8_000;
...
function buildCloudFolderStatus(workspaceRoots) {
  ...
  if (connection.provider === 'icloud') {
    ...
    const containerFolder = ICLOUD_CONTAINER_ID.replace(/\./g, '~');
    finderPath = `~/Library/Mobile Documents/${containerFolder}/Documents/${workspaceRelativePath}`;
  }
...
export function getCloudFolderStatus(workspaceRoots, options = {}) {
  ...
  const connection = loadConnection();
  const cacheMs =
    connection?.provider === 'icloud' ? FOLDER_STATUS_ICLOUD_CACHE_MS : FOLDER_STATUS_CACHE_MS;
  if (
    !options.force &&
    folderStatusCache &&
    folderStatusCache.key === cacheKey &&
    Date.now() - folderStatusCache.at < cacheMs
  ) {
