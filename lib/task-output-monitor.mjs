...
import { readComposerConversationTextAsync, readComposerProgressAsync } from './composer-output.mjs';
import { isDailyCursorUsageBudgetFailure } from './response-failure.mjs';
import { syncTaskComposerAttachments, syncTaskComposerAttachmentsWithRetry } from './task-attached-files.mjs';
...
async function syncComposerAttachments(taskFolder, composerId, options = {}) {
  const syncFn =
    options.maxAttempts && options.maxAttempts > 1
      ? syncTaskComposerAttachmentsWithRetry
      : syncTaskComposerAttachments;
  const saved = await syncFn(taskFolder, composerId, options);
  if (saved.length > 0) {
    log(`Synced ${saved.length} attachment(s) from composer ${composerId}.`);
  }
}

async function isStillTrackedComposer(taskFolder, composerId) {
  const currentComposerId = await readTaskField(taskFolder, 'readComposerId');
  if (currentComposerId === composerId) {
    return true;
  }

  log(
    `Skipping terminal update from stale composer ${composerId}; ` +
      `task now tracks ${currentComposerId ?? 'none'}.`,
  );
...
async function finalizeDone(taskFolder, composerId, responseText) {
  if (!(await isStillTrackedComposer(taskFolder, composerId))) {
    return;
  }

  await syncComposerAttachments(taskFolder, composerId, {
    maxAttempts: 8,
    delayMs: 1_500,
  });
  ...
  if (!finalResponseText) {
    finalResponseText = await readComposerConversationTextAsync(composerId, {
      maxChars: 200_000,
    });
  }

  if (finalResponseText) {
    await writeTaskField(taskFolder, 'writeResponseText', finalResponseText);
  }
  ...
  trackerByComposerId.delete(composerId);
  log(`Task done; saved ${finalResponseText.length} chars from composer ${composerId}.`);
}
...
export async function pollRunningTaskOutput(taskFolder) {
  ...
  // Persist generated images and other chat attachments on every poll. Tool
  // bubbles (e.g. generate_image) are not reflected in headerCount, so a
  // fingerprint-only sync can miss assets before the agent tab is closed.
  await syncComposerAttachments(taskFolder, composerId);
